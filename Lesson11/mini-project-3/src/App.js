...
function App() {
  ...
  // cách 1: lưu trữ dự liệu bằng UseState
  // const [tasks, setTasks] = useState(listTaskInit);

  // cách 2:lưu trữ dự liệu bằng UseState
  const [tasks, setTasks] = useState(()=>{
    // đọc dữ liệu từ localStorage hoặc API
  ...
  // useEffect: khi tasks thjay đổi thì cập nhật vào localStorage
  useEffect(()=>{
    localStorage.setItem("DEV2407RJ_TASKS",JSON.stringify(tasks) )
  },[tasks])
  ...
  // trạn thái hiển thị giá trị của nút submit của form : Save(add)/ Update (Edit)
  ...
  let elementForm  = isToggle === true ?(<Form onCancel = {handleCancelForm}
    renderTask = {task}
